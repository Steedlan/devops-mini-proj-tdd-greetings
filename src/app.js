...
function greet(name) {
    ...
    if(!isUndefinedOrNull(name) && isAnArray(name) && name.length === 1) name = name[0];
    
    if(isUndefinedOrNull(name) || isEmptyString(name)) return `${language.helloLower}, ${language.isNull}`;
...
function arrayGreeting(name, language) {
    ...
    for(i = 0; i < name.length ; i += 1) {
        ...
        if(i === name.length-1) message += ` ${language.andLower} ${name[i]}.`;
        else message += `, ${name[i]}`;
    }
    if(upperCase!==undefined) message+= ` ${language.andUpper} ${language.helloUpper} ${upperCase}!`;
    return message;
}
